...
def check_duplicate_member(email: str):
    """Check if this email is already registered with a paid registration.

    Prevents the same person from registering (and paying) twice.
    """
    result = (
        supabase.table("members")
        .select("id, registration_id!inner(id, payments!inner(status))")
        .eq("email", email)
        .eq("registration_id.payments.status", "paid")
        .limit(1)
        .execute()
    )
    if result.data:
        raise ValueError(f"A member with this email is already registered.")


def create_registration(data: RegistrationInput) -> dict:
    """Save registration + members to DB. Returns data needed for QR/email processing.

    Called by the webhook handler AFTER payment succeeds.
    Quota check and duplicate check happen before this function is called.

    Flow:
      1. Check for duplicate members
      2. Insert into 'registrations' table (seq auto-generated by DB)
      3. Read back the DB-generated seq, build reference like "HP-2026-00042"
      4. Insert each member into 'members' table
      5. If any member insert fails, delete the registration (CASCADE cleans up)
      6. Return data dict for QR code generation and email sending
    """
    # Check duplicate members (only those with email)
    for member in data.members:
        if member.email:
            check_duplicate_member(member.email)
